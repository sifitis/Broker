...
$tellraw @s [{"text":"View","color":"#1CEEEE",\
    "click_event":{"action":"run_command","command":"/data get storage datapack:broker data.component_manifest.$(entry_id)"}\
    },{"text":" - View the entry data.","color":"white"}]

$tellraw @s [{"text":"Delete","color":"red",\
    "click_event":{"action":"run_command","command":"/function broker:util/manifest/delete_entry {entry_id:$(entry_id)}"}\
